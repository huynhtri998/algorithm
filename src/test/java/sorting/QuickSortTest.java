...
public class QuickSortTest {
    ...
    private int[] callQuickSort(int[] arrInput) {
        if (arrInput.length == 0) {
            return quickSort.quickSort(arrInput, 0, 0);
        }

        return quickSort.quickSort(arrInput, 0, arrInput.length - 1);
    }

    @Test
    void normalQuickSort() {
        int[] arrInput = {4,1,6,2,5,3,9,7,8};
        int[] expectedOutput = {1,2,3,4,5,6,7,8,9};
        int[] result = callQuickSort(arrInput);
